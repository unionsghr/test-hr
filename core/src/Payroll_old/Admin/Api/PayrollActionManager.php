...
////////////////////                 SYSTEM     ////////////////////////////
    ...
    public function getAllData($req)
    {
        ...
        foreach ($baseEmpList as $baseEmp) {
            $employeeNamesById[$baseEmp->id] = $baseEmp->first_name." ".$baseEmp->middle_name." ".$baseEmp->last_name;
            $empIds[] = $baseEmp->id;
        }
    ...
    // Payslip Mail
    public function payslipmail($req)
    {
        
        if ($this->user->user_level == 'Admin') {
            $user = new Employee();
            $user->Load("work_email = ?", array($req->work_email));

            if ($user->work_email == $req->work_email) {
            ...
            $user = new Employee();
            $user->work_email = $req->work_email;
            ...
            $user = new Payroll();
            $user->name = $req->name;
            $user->date_start = $req->date_start;
            $user->date_end = $req->date_end;
            ...
            if (!empty($this->emailSender)) {
                $usersEmailSender = new PayrollEmailSender($this->emailSender, $this);
                $mailResponse = $usersEmailSender->sendPayrollEmail($user, $employee);
            }
